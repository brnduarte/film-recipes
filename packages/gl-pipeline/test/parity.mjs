...
for (const { name, pixels, manual } of golden) {
  // "<recipe>+manual" / "<recipe>+grade" / "<recipe>+overlay" fixtures reuse
  // the base recipe params and layer on the golden's manual grade / color
  // grade / overlay (dump_recipe_golden.rs) — strip the suffix to look up
  // the shared recipe fixture.
  const recipe = RECIPES[name.replace(/\+(manual|grade|overlay)$/, "")];
  if (!recipe) {
